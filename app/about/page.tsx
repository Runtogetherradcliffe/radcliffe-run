...
function StatCard({ value, label, sub }: { value: string; label: string; sub?: string }) {
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px 20px', textAlign: 'center' }}>
      <p style={{ fontSize: 36, fontWeight: 800, letterSpacing: '-0.03em', color: '#f5a623', lineHeight: 1, marginBottom: 8 }}>{value}</p>
      <p style={{ fontSize: 'var(--text-base)', fontWeight: 600, color: 'var(--dim)', marginBottom: sub ? 4 : 0 }}>{label}</p>
      {sub && <p style={{ fontSize: 12, color: 'var(--faint)' }}>{sub}</p>}
    </div>
  )
}
...
function InfoRow({ label, value, href }: { label: string; value: string; href?: string }) {
  return (
    <div style={{ padding: '14px 0', borderBottom: '1px solid var(--border)' }}>
      <p style={{ fontSize: 'var(--text-xs)', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--faint)', marginBottom: 3 }}>{label}</p>
      {href ? (
        <a href={href} target="_blank" rel="noopener noreferrer" style={{ fontSize: 'var(--text-md)', color: 'var(--dim)', lineHeight: 1.5, textDecoration: 'none' }}>{value}</a>
      ) : (
        <p style={{ fontSize: 'var(--text-md)', color: 'var(--dim)', lineHeight: 1.5 }}>{value}</p>
      )}
    </div>
  )
}
  ...
  return (
    <>
      ...
      <main>
        ...
        {/* ── Hero ── */}
        <section style={{ maxWidth: 780, margin: '0 auto', padding: '56px 24px 48px', borderBottom: '1px solid var(--border)' }}>
          <p style={{ fontSize: 'var(--text-xs)', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#f5a623', marginBottom: 12 }}>About us</p>
        ...
        {/* ── Stats ── */}
        <section style={{ maxWidth: 780, margin: '0 auto', padding: '48px 24px', borderBottom: '1px solid var(--border)' }}>
        ...
        <section style={{ maxWidth: 780, margin: '0 auto', padding: '48px 24px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'var(--about-2col)', gap: 'var(--about-2col-gap)', alignItems: 'start' }}>
            ...
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: '24px 20px' }}>
              <p style={{ fontSize: 'var(--text-xs)', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--faint)', marginBottom: 4 }}>The essentials</p>
              <div>
        ...
        {/* ── Pace groups ── */}
        <section id="groups" style={{ maxWidth: 780, margin: '0 auto', padding: '0 24px 48px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ marginBottom: 28 }}>
            <p style={{ fontSize: 'var(--text-xs)', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#f5a623', marginBottom: 10 }}>Groups</p>
        ...
        <section style={{ maxWidth: 780, margin: '0 auto', padding: '48px 24px' }}>
          <div style={{ marginBottom: 32 }}>
            <p style={{ fontSize: 'var(--text-xs)', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#f5a623', marginBottom: 10 }}>FAQ</p>
        ...
        {/* ── Join CTA ── */}
        <section style={{ background: 'var(--card-hi)', borderTop: '1px solid var(--border)' }}>
          <div style={{ maxWidth: 780, margin: '0 auto', padding: '64px 24px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
              <a href="/join" style={{
                ...
                background: '#f5a623', color: '#0a0a0a', fontSize: 'var(--text-md)', fontWeight: 700,
