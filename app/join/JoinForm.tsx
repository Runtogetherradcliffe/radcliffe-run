...
interface FormData {
  ...
  c25kSession: 'tuesday' | 'thursday' | 'both' | ''
}
...
const labelStyle: React.CSSProperties = {
  display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--muted)',
...
function ConsentItem({ checked, onChange, required, children }: {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 'var(--text-base)', fontWeight: 300, color: 'var(--dim)', lineHeight: 1.6 }}>
        {children}
...
/* ── Session picker (C25K only) ── */
function SessionPicker({ value, onChange }: {
  value: FormData['c25kSession']
  onChange: (v: FormData['c25kSession']) => void
}) {
  const options: { v: FormData['c25kSession']; label: string; sub: string }[] = [
    { v: 'tuesday',  label: 'Tuesday',       sub: 'Evenings at 7pm' },
    { v: 'thursday', label: 'Thursday',      sub: 'Evenings at 7pm' },
    { v: 'both',     label: 'Both',          sub: 'Tuesday & Thursday' },
  ]
  return (
    <div>
      <label style={labelStyle}>
        Which sessions can you attend?
        <span style={{ color: '#f5a623', marginLeft: 3 }}>*</span>
      </label>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
        {options.map(o => (
          <button
            ...
            style={{
              background: value === o.v ? 'rgba(245,166,35,0.1)' : '#0a0a0a',
              border: value === o.v ? '1.5px solid #f5a623' : '1.5px solid #222',
          ...
          >
            <p style={{
              ...
              color: value === o.v ? '#f5a623' : '#ccc',
              margin: '0 0 3px', fontFamily: 'Inter, sans-serif',
            }}>
              {o.label}
            </p>
            <p style={{ fontSize: 'var(--text-xs)', fontWeight: 300, color: 'var(--faint)', margin: 0, fontFamily: 'Inter, sans-serif' }}>
              {o.sub}
            </p>
          </button>
        ))}
...
/* ── Step 1: About you ── */
function StepAboutYou({ data, onChange, isC25K }: {
  data: FormData
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {isC25K && (
        <SessionPicker
          value={data.c25kSession}
          onChange={v => onChange('c25kSession', v)}
        />
      )}
      <p style={{ fontSize: 'var(--text-sm)', color: 'var(--faint)', lineHeight: 1.6, paddingTop: 4 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ background: 'rgba(245,166,35,0.06)', border: '1px solid rgba(245,166,35,0.15)', borderRadius: 10, padding: '14px 16px', marginBottom: 4 }}>
        <p style={{ fontSize: 'var(--text-sm)', fontWeight: 500, color: 'var(--dim)', lineHeight: 1.6 }}>
    ...
    >
      <div
        ...
        style={{
          ...
          background: 'var(--card)', border: '1px solid var(--border-2)', borderRadius: 16,
      ...
      >
        <div style={{
          ...
          <p style={{ fontSize: 'var(--text-base)', fontWeight: 600, color: 'var(--dim)' }}>Privacy Policy</p>
          <button
            onClick={onClose}
            style={{
              background: 'var(--card-hi)', border: '1px solid var(--border-2)', color: 'var(--muted)',
...
function StepLastBits({ data, onChange, onToggle }: {
  ...
  return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <p style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--muted)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 10 }}>Health</p>
      ...
      <div>
        <p style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--muted)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 10 }}>Age</p>
        <div style={{ fontSize: 'var(--text-sm)', color: 'var(--muted)', lineHeight: 1.6, marginBottom: 10, padding: '10px 14px', background: '#0d0d0d', borderRadius: 8, border: '1px solid var(--border)' }}>
      ...
      <div>
        <p style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--muted)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 10 }}>Your data</p>
      ...
      <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 24px', textAlign: 'left' }}>
        <form onSubmit={handleVerify} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <input
            ...
            style={{ width: '100%', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 14px', fontSize: 24, color: 'var(--white)', fontFamily: 'Inter, sans-serif', outline: 'none', letterSpacing: '0.2em', textAlign: 'center', boxSizing: 'border-box' }}
          />
          {error && <p style={{ fontSize: 'var(--text-sm)', color: '#e05252' }}>⚠️ {error}</p>}
          <button
            type="submit"
            disabled={loading || code.length < 6}
            style={{ background: loading || code.length < 6 ? '#1a1a1a' : '#f5a623', color: loading || code.length < 6 ? '#333' : '#0a0a0a', fontSize: 'var(--text-base)', fontWeight: 700, padding: '12px 24px', borderRadius: 8, border: 'none', cursor: loading || code.length < 6 ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}
          >
...
/* ── Step 4: Welcome ── */
function StepWelcome({ name, email, nextRun, onReset, isC25K, c25kSession }: {
  name: string; email: string; nextRun: NextRun; onReset: () => void
  isC25K: boolean; c25kSession: FormData['c25kSession']
}) {
  const [code,    setCode]    = useState('')
  const [loading, setLoading] = useState(false)
  const [error,   setError]   = useState<string | null>(null)
  ...
  return (
    <div style={{ textAlign: 'center', padding: '16px 0' }}>
      ...
      {/* Next run card */}
      {nextRun && !isC25K && (
        <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 24px', marginBottom: 28, textAlign: 'left' }}>
          <p style={{ fontSize: 'var(--text-xs)', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#f5a623', marginBottom: 8 }}>Next run</p>
      ...
      {isC25K && (
        <div style={{ background: 'var(--bg)', border: '1px solid rgba(245,166,35,0.2)', borderRadius: 12, padding: '20px 24px', marginBottom: 28, textAlign: 'left' }}>
          <p style={{ fontSize: 'var(--text-xs)', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#f5a623', marginBottom: 8 }}>Your C25K plan</p>
          <p style={{ fontSize: 'var(--text-sm)', fontWeight: 300, color: 'var(--muted)', lineHeight: 1.6, margin: 0 }}>
            <strong style={{ color: 'var(--dim)' }}>Sessions:</strong> {sessionText}<br />
            <strong style={{ color: 'var(--dim)' }}>Meeting point:</strong> Radcliffe Market, Blackburn Street, M26 1PN<br />
            <strong style={{ color: 'var(--dim)' }}>Start time:</strong> 7:00pm
          </p>
        </div>
      )}

      {/* Sign-in code entry */}
      <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 24px', marginBottom: 20, textAlign: 'left' }}>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--dim)', marginBottom: 16, lineHeight: 1.6 }}>
          We&apos;ve sent a sign-in code to <strong style={{ color: 'var(--dim)' }}>{email}</strong>. Enter it below to go straight to your profile.
        </p>
        <form onSubmit={handleVerify} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <input
            type="text"
      ...
      <p style={{ fontSize: 'var(--text-sm)', color: 'var(--faint)', marginBottom: 8 }}>
        <Link href="/routes" style={{ color: 'var(--faint)' }}>Explore the routes</Link>
      </p>
      <p style={{ fontSize: 'var(--text-sm)', color: 'var(--faint)' }}>
        <button onClick={onReset} style={{ background: 'none', border: 'none', color: 'var(--faint)', cursor: 'pointer', textDecoration: 'underline', fontSize: 'var(--text-sm)', fontFamily: 'inherit' }}>
...
/* ── STEP CONFIG ── */
function getSteps(isC25K: boolean) {
  return [
    { label: isC25K ? 'About you'         : 'About you',         subtitle: isC25K ? 'Tell us about yourself and which sessions you can make.' : 'Tell us a bit about yourself.' },
    { label: 'Emergency contact', subtitle: 'Someone we can call if needed.' },
    { label: 'Last bits',         subtitle: 'Health and data preferences.' },
  ]
}

function isStepValid(step: number, data: FormData, isC25K: boolean): boolean {
  if (step === 0) return !!(
    data.firstName && data.lastName &&
    data.email && isValidEmail(data.email) &&
    (!data.mobile || isValidUKPhone(data.mobile)) &&
    (!isC25K || data.c25kSession !== '')
  )
  if (step === 1) return !!(
    data.emergencyName &&
...
/* ── PAGE ── */
export default function JoinForm({ isC25K }: { isC25K: boolean }) {
  ...
  const handleContinue = async () => {
    ...
    try {
      const res = await fetch('/api/join', {
        ...
        body: JSON.stringify({
          ...data,
          cohort: isC25K ? 'c25k' : null,
          c25kSession: isC25K ? data.c25kSession : null,
        }),
      })
  ...
  const valid = isStepValid(step, data, isC25K)

  return (
    <>
      <Nav />
      <main style={{ minHeight: 'calc(100vh - 60px)', background: 'var(--bg)', padding: 'var(--join-outer-pad)' }}>
        <div style={{ maxWidth: 480, margin: '0 auto' }}>
          ...
          {isC25K && !done && (
            <div style={{
              ...
              <p style={{ fontSize: 'var(--text-sm)', fontWeight: 500, color: '#f5a623', margin: 0 }}>
          ...
          {!done && (
            <div style={{ textAlign: 'center', marginBottom: 40 }}>
              ...
              <p style={{ fontSize: 'var(--text-md)', fontWeight: 300, color: 'var(--muted)' }}>{STEPS[step].subtitle}</p>
            </div>
          )}
          ...
          {/* Form card */}
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, padding: 'var(--join-card-pad)', boxShadow: '0 24px 48px rgba(0,0,0,0.4)' }}>
            {alreadyRegistered ? (
              ...
              <StepWelcome
                ...
                onReset={handleReset}
                isC25K={isC25K}
                c25kSession={data.c25kSession}
              />
            ) : (
              <>
                {step === 0 && <StepAboutYou   data={data} onChange={update} isC25K={isC25K} />}
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 28, alignItems: 'center' }}>
                  {step > 0 && (
                    <button onClick={() => setStep(s => s - 1)} disabled={submitting} style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--muted)', fontSize: 'var(--text-base)', fontWeight: 500, padding: '11px 20px', borderRadius: 8, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}>
                      Back
                    </button>
                  )}
                  <button
                ...
                {submitError && (
                  <p style={{ fontSize: 'var(--text-sm)', color: '#e05252', marginTop: 12, textAlign: 'center' }}>
                    ⚠️ {submitError}
                  </p>
                )}
          ...
          {!done && (
            <p style={{ textAlign: 'center', fontSize: 'var(--text-sm)', color: 'var(--faint)', marginTop: 20 }}>
              Already registered? <Link href="/profile" style={{ color: 'var(--muted)', textDecoration: 'underline' }}>Sign in</Link>
            </p>
          )}
        </div>
      </main>
